		...
		<div class="mx-auto max-w-7xl px-6 py-8">
			...
			{#if projectId}
				<div class="{darkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'} rounded-lg border shadow-sm" in:fade={{ duration: 300, delay: 300 }}>
					<div class="p-6">
						{#if isLoadingEntities && documentEntitiesManager === null}
						...
						{:else}
							{#if isLoadingEntities}
								<p class="mb-3 text-xs {darkMode ? 'text-slate-400' : 'text-slate-500'} flex items-center gap-2" aria-live="polite">
									<svg class="h-3.5 w-3.5 animate-spin shrink-0" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
										<circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
										<path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
									</svg>
