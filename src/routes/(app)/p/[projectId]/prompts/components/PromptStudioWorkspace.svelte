<script lang="ts">
	...
	import PromptEditModal from './PromptEditModal.svelte';
	import JsonSchemaPickerModal from '$lib/components/schemas/JsonSchemaPickerModal.svelte';
	import ConfirmModal from '$lib/components/ui/ConfirmModal.svelte';
	import PromptWorkspaceCenter from './PromptWorkspaceCenter.svelte';
	import PromptChatSidebar from './PromptChatSidebar.svelte';
	import PromptLibrarySidebar from './PromptLibrarySidebar.svelte';
	...
	interface Props {
		...
		/** When true, fills parent (e.g. modal) instead of viewport height; hides Prompt Library top bar. */
		embedded?: boolean;
		/** Called after a successful “send to dashboard” when `embedded` is true (e.g. close modal). */
		onSendToDashboardSuccess?: () => void;
		/** Label for chat panel dashboard action (default matches standalone Prompt Library). */
	...
	async function handleAddChatResponseToDashboard(text: string, tabId?: string): Promise<boolean> {
		...
		if (!pid || !tok || !text.trim()) return false;
		await dashboard.initialize(pid, tok);
		const title = selectedWorkspacePrompt?.name?.trim() || 'Prompt response';
		const ok = addPlainParagraphToDashboard(text, pid, title, tabId);
		if (ok && embedded && onSendToDashboardSuccess) {
			onSendToDashboardSuccess();
		}
		return ok;
	}
	...
	const promptsAddChatToDashboard: PromptsAddChatToDashboardContext = {
		add: (text, tabId) => handleAddChatResponseToDashboard(text, tabId)
	};
	...
	function handleEditTemplate(template: Prompt) {
		...
		const templateStr = getTemplateStrForEditor(template);
		const parsed = parseTemplateToAIQueryData(templateStr ?? '');
		workspaceQuestion = (parsed.prompt ?? '').trim();
...
<div
	class="flex w-full flex-col overflow-hidden transition-colors {embedded
		? 'min-h-0 h-full flex-1'
		: 'h-screen'} {darkMode ? 'bg-slate-900' : 'bg-slate-50'}"
>
	{#if !embedded}
	...
	<div class="flex min-h-0 flex-1 overflow-hidden">
		...
		{:else}
			...
			<div class="flex min-h-0 shrink-0 flex-col overflow-hidden" style="width: {rightSidebarWidth}px">
				<PromptChatSidebar
					{darkMode}
					projectId={selectedProjectId ?? ''}
					selectedPrompt={selectedWorkspacePrompt}
