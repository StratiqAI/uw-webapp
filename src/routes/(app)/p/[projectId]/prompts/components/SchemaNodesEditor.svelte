<script lang="ts">
	 ...
	 * Used by PromptEditModal flat "Fields to get back" for object and array item shapes.
	 */
	import type {
		NestedItemSchema,
		...
		onNodesChange(
			nodes.map((n) => {
				...
				if (n.itemSchema?.type === 'object') {
					const props = n.itemSchema.properties ?? [];
					// Allow adding to item object root: parentId could be synthetic — use node id for array item object
	...
	function addItemProperty(
		...
		// parentId matches array node id: append to item.properties when item is object
		if (item.type === 'object') {
			...
			<div class="rounded border {darkMode ? 'border-gray-600 bg-slate-800' : 'border-gray-300 bg-white'} p-3">
				...
				{#if isArray && node.itemSchema}
					<div class="ml-2 mt-2 rounded border {darkMode ? 'border-gray-600' : 'border-gray-300'} border-dashed p-2">
						<span class="mb-1 block text-xs font-medium {darkMode ? 'text-slate-400' : 'text-slate-600'}"
							>Array item shape</span
						>
						<select
