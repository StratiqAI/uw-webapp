...
<div class="space-y-4">
	<h2 class="text-xl font-semibold dark:text-slate-100">Store Contents</h2>

	<!-- Store Tree - Full JSON representation of all stored data -->
	<div class="rounded-md border border-gray-300 dark:border-gray-600 bg-white dark:bg-slate-800 p-4">
		<strong class="mb-2 block dark:text-slate-200">Store Tree:</strong>
		<!-- Display entire store tree as formatted JSON for debugging -->
		<pre
			class="mt-2 max-h-96 overflow-auto rounded border bg-gray-50 dark:bg-slate-900 dark:border-slate-700 dark:text-slate-200 p-3 text-xs">{JSON.stringify(
				storeTree,
				null,
				2
			)}</pre>
