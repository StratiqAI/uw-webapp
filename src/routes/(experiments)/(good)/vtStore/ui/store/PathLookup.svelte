...
<div class="rounded-md border border-purple-300 bg-purple-50 p-4">
	<strong class="mb-2 block">Path Lookup:</strong>
	...
	{#if lookupPath}
		<div class="mt-4 space-y-2">
			...
			<div class="text-sm text-gray-600">
				<strong>Path:</strong> <code class="rounded bg-gray-200 px-1">{lookupPath}</code>
			</div>
			...
			{#if pathErrors}
				<div class="rounded-md border border-red-300 bg-red-50 p-3">
					...
					<pre class="mt-2 overflow-auto text-xs text-red-600">{JSON.stringify(
							pathErrors,
							null,
							2
						)}</pre>
			...
			{#if pathValue !== null && pathValue !== undefined}
				<div class="rounded-md border border-green-300 bg-green-50 p-3">
					<strong class="text-green-700">Value:</strong>
					<pre
						class="mt-2 max-h-64 overflow-auto rounded border bg-white p-2 text-xs">{JSON.stringify(
							pathValue,
							null,
							2
						)}</pre>
			...
			{:else if lookupPath}
				...
				<div class="rounded-md border border-yellow-300 bg-yellow-50 p-3 text-yellow-700">
