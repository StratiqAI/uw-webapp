<script lang="ts">
	...
	const workspaceBase = $derived(`/projects/workspace/${project.id}`);

	const workspacePages = [
		{ label: 'Documents', segment: 'document-analysis', icon: 'doc' },
		{ label: 'Deal Room', segment: 'deal-room', icon: 'briefcase' },
		{ label: 'Financial', segment: 'financial-analysis', icon: 'chart' },
		{ label: 'Market', segment: 'market-analysis', icon: 'globe' },
		{ label: 'Reports', segment: 'reports', icon: 'report' },
		{ label: 'Insights', segment: 'insights', icon: 'lightbulb' }
	];
...
>
	...
	<div class="flex items-start gap-3 p-4 pb-2">
		<a
			href={`${workspaceBase}/document-analysis`}
	...
	<div class="flex items-center gap-1.5 px-4 py-2.5 border-t {darkMode ? 'border-slate-700/50' : 'border-slate-200/60'} flex-wrap">
		{#each workspacePages as wp}
			<a
				href={`${workspaceBase}/${wp.segment}`}
