		...
		{#each images as image (image.id)}
			...
			<div class="p-3 {darkMode ? 'bg-purple-900/20 border-purple-500/30 hover:bg-purple-900/30' : 'bg-purple-50 border-purple-200 hover:bg-purple-100'} rounded-lg border transition-colors">
				...
				<SendToDashboardButton
					{darkMode}
					colorClasses={darkMode ? 'text-purple-400 hover:text-purple-300' : 'text-purple-600 hover:text-purple-700'}
					onSend={(tabId) => addImageToDashboard(image, projectId, tabId)}
				/>
			</div>
		{/each}
	</div>
