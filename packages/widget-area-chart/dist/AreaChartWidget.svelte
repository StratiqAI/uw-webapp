<script lang="ts">
	import type { AreaChartWidgetData } from './schema.js';
	import {
	...
	let {
		data,
		widgetId = 'area-widget-default',
		topicOverride,
		darkMode = false
	}: StandardWidgetProps<AreaChartWidgetData> = $props();

	const host = getDashboardWidgetHost();
	const topic = $derived(host.getWidgetTopic('areaChart', widgetId, topicOverride));
	const dataStream = useReactiveValidatedTopic<AreaChartWidgetData>(() => topic);
	const widgetData = $derived<AreaChartWidgetData>(
		dataStream.current ||
			data || {
				labels: [],
				datasets: []
