<script lang="ts">
	...
	let weightEmp = $state(35);
	let weightLq = $state(30);
	let weightWage = $state(20);
	let weightEstab = $state(15);

	$effect(() => {
		weightEmp = widgetData.weights.emp;
		weightLq = widgetData.weights.lq;
		weightWage = widgetData.weights.wage;
		weightEstab = widgetData.weights.estab;
	});

	function persistWeights() {
		onUpdateConfig?.({
			...widgetData,
			weights: { emp: weightEmp, lq: weightLq, wage: weightWage, estab: weightEstab }
		});
	}
	...
	const scoredIndustries = $derived.by((): ScoredIndustry[] => {
		...
		const totalWeight = weightEmp + weightLq + weightWage + weightEstab;
		if (totalWeight === 0) return items.map((i) => ({ ...i, score: 0 }));

		return items.map((item, idx) => {
			const lqN = lqTrendScore(item.industry.lqTrend);
			const composite =
				(empNorm[idx] * weightEmp +
					lqN * weightLq +
					wageNorm[idx] * weightWage +
					estabNorm[idx] * weightEstab) /
				totalWeight;
...
<div class="flex h-full flex-col overflow-hidden rounded-lg border shadow-sm {shell}">
	...
	<div class="shrink-0 border-t px-4 py-2.5 {borderColor}">
		<div class="flex flex-wrap items-center gap-x-6 gap-y-2">
			<span class="text-[10px] font-bold uppercase tracking-wider {muted}">Score Weights:</span>
			<label class="inline-flex items-center gap-2">
				<span class="text-[10px] {muted}">Emp</span>
				<input
					type="range" min="0" max="100" value={weightEmp}
					oninput={(e) => { weightEmp = Number(e.currentTarget.value); persistWeights(); }}
					class="h-1.5 w-20 cursor-pointer appearance-none rounded-full"
					style="accent-color: #22d3ee"
				/>
				<span class="w-8 text-right font-mono text-[10px] font-bold tabular-nums {muted}">{weightEmp}%</span>
			</label>
			<label class="inline-flex items-center gap-2">
				<span class="text-[10px] {muted}">LQ</span>
				<input
					type="range" min="0" max="100" value={weightLq}
					oninput={(e) => { weightLq = Number(e.currentTarget.value); persistWeights(); }}
					class="h-1.5 w-20 cursor-pointer appearance-none rounded-full"
					style="accent-color: #3b82f6"
				/>
				<span class="w-8 text-right font-mono text-[10px] font-bold tabular-nums {muted}">{weightLq}%</span>
			</label>
			<label class="inline-flex items-center gap-2">
				<span class="text-[10px] {muted}">Wage</span>
				<input
					type="range" min="0" max="100" value={weightWage}
					oninput={(e) => { weightWage = Number(e.currentTarget.value); persistWeights(); }}
					class="h-1.5 w-20 cursor-pointer appearance-none rounded-full"
					style="accent-color: #10b981"
				/>
				<span class="w-8 text-right font-mono text-[10px] font-bold tabular-nums {muted}">{weightWage}%</span>
			</label>
			<label class="inline-flex items-center gap-2">
				<span class="text-[10px] {muted}">Estab</span>
				<input
					type="range" min="0" max="100" value={weightEstab}
					oninput={(e) => { weightEstab = Number(e.currentTarget.value); persistWeights(); }}
					class="h-1.5 w-20 cursor-pointer appearance-none rounded-full"
					style="accent-color: #a855f7"
				/>
				<span class="w-8 text-right font-mono text-[10px] font-bold tabular-nums {muted}">{weightEstab}%</span>
			</label>
		</div>
	</div>
</div>
