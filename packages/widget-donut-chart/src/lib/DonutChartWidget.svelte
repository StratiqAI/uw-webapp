<script lang="ts">
	import type { DonutChartWidgetData } from './schema.js';
	import {
	...
	let {
		data,
		widgetId = 'donut-widget-default',
		topicOverride,
		darkMode = false
	}: StandardWidgetProps<DonutChartWidgetData> = $props();

	const host = getDashboardWidgetHost();
	const topic = $derived(host.getWidgetTopic('donutChart', widgetId, topicOverride));
	const dataStream = useReactiveValidatedTopic<DonutChartWidgetData>(() => topic);
	const widgetData = $derived<DonutChartWidgetData>(
		dataStream.current ||
			data || {
				labels: [],
				values: []
