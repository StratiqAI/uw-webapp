 ...
 */
export function npv(discountRate: number, cashFlows: readonly number[]): number {
 ...
 */
export function equityMultiple(cashFlows: readonly number[]): number | null {
	if (cashFlows.length < 2) return null;
	const y0 = cashFlows[0]!;
	const neg = y0 < 0 ? Math.abs(y0) : 0;
	if (neg < 1e-10) return null;
	let pos = 0;
	for (let t = 1; t < cashFlows.length; t++) {
		const c = cashFlows[t]!;
		if (c > 0) pos += c;
 ...
 */
export function cashOnCash(year0Equity: number, year1Cash: number): number | null {
