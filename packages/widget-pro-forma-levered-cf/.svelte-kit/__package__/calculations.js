...
function remainingBalance(principal, annualRate, amortYears, paymentsMade) {
    const n = amortYears * 12;
...
export function computeLeveredProjections(config) {
    const unlevered = computeUnleveredProjections(config);
    const N = config.projectionYears;
    const totalAcquisition = config.purchasePrice + config.acquisitionCosts + config.initialCapEx;
    ...
    if (!config.interestOnly && loanAmount > 0) {
        remainingLoanAtSale = remainingBalance(loanAmount, config.loanInterestRate, config.amortizationYears, N * 12);
    }
    ...
    const netSaleAfterPayoff = exitCapValid && unleveredNetSaleYearN > 0
    ...
    for (let year = 0; year <= N; year++) {
        ...
        const saleAfter = year === N && exitCapValid ? netSaleAfterPayoff : 0;
        let leveredCf;
        if (year === 0) {
...
export function extractLeveredCashFlows(result) {
