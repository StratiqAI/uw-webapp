 ...
 */
function noiYearAfterHold(N: number, cfg: ProFormaUnleveredCfConfig): number {
...
export function computeUnleveredProjections(
	config: ProFormaUnleveredCfConfig
): { columns: UnleveredYearColumn[]; grossSalePrice: number | null; year6Noi: number; exitCapValid: boolean } {
	...
	let grossSalePrice: number | null = null;
	...
	const columns: UnleveredYearColumn[] = [];

	for (let year = 0; year <= N; year++) {
		const acq = year === 0 ? acquisitionTotal : 0;
		const noi = noiForOperatingYear(
			year,
			config.egiYear1,
			config.egiGrowthRate,
			config.totalOpexYear1,
			config.opexGrowthRate
		);
...
export function extractUnleveredCashFlows(
	result: ReturnType<typeof computeUnleveredProjections>
): number[] {
