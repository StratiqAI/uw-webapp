<script lang="ts">
	import type { SparklineWidgetData } from './schema.js';
	import {
	...
	let {
		data,
		widgetId = 'sparkline-widget-default',
		topicOverride,
		darkMode = false
	}: StandardWidgetProps<SparklineWidgetData> = $props();

	const host = getDashboardWidgetHost();
	const topic = $derived(host.getWidgetTopic('sparkline', widgetId, topicOverride));
	const dataStream = useReactiveValidatedTopic<SparklineWidgetData>(() => topic);
	const widgetData = $derived<SparklineWidgetData>(
		dataStream.current ||
			data || {
				values: []
			}
